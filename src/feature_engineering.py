...
VELOCITY_WINDOW = "30min"
COUNTRY_RISK_SMOOTHING_WEIGHT = 10
COUNTRY_RISK_DEFAULT_FOR_UNSEEN_TARGET = 0.01
NO_PRIOR_TRANSACTION_SENTINEL_SECONDS = 999_999


def calculate_country_risk(
    df: pd.DataFrame, target_series: Optional[pd.Series] = None
) -> pd.DataFrame:
    """
    ...
    if target_series is not None:
        ...
        smooth_weight = COUNTRY_RISK_SMOOTHING_WEIGHT

        smooth_risk = (counts * means + smooth_weight * global_mean) / (
            counts + smooth_weight
        )
        df["country_fraud_risk"] = df["country"].map(smooth_risk).fillna(global_mean)
    else:
        df["country_fraud_risk"] = COUNTRY_RISK_DEFAULT_FOR_UNSEEN_TARGET
...
def engineer_velocity_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    days = df["time_since_signup"] / SECONDS_PER_DAY
    days_safe = days.replace(0, 1)
    df["user_txn_velocity"] = df["user_txn_count"] / days_safe

    # 2. Platform-wide multi-account velocity tracking window
    df["_orig_order_idx"] = np.arange(len(df))
    v_df = df[["_orig_order_idx", "browser", "country", "purchase_time"]].sort_values(
        "purchase_time"
    )
    ...
    merged = pd.merge_asof(
        ...
        on="purchase_time",
        by=["browser", "country"],
        tolerance=pd.Timedelta(VELOCITY_WINDOW),
    ...
    print(
        f"[feature_engineering] platform_30min_velocity: mean={df['platform_30min_velocity'].mean():.2f} max={df['platform_30min_velocity'].max()}"
    )
    return df
...
def engineer_behavior_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["account_age_days"] = df["time_since_signup"] / SECONDS_PER_DAY
    hours = df["time_since_signup"] / SECONDS_PER_HOUR
    ...
    cohort_avg = df.groupby(["browser", "country"])["purchase_value"].transform(
        "median"
    )
    ...
    df["time_since_prev_txn"] = df["time_since_prev_txn"].fillna(
        NO_PRIOR_TRANSACTION_SENTINEL_SECONDS
    )

    return df


def engineer_all(df: pd.DataFrame, y_train: Optional[pd.Series] = None) -> pd.DataFrame:
    """
    ...
    target_series = (
        ...
        else (df["class"] if "class" in df.columns else None)
    )
    df = calculate_country_risk(df, target_series=target_series)

    new_cols = set(df.columns) - original_cols
    print(
        f"[feature_engineering] Completed! Features added ({len(new_cols)}): {sorted(new_cols)}"
    )
    return df
