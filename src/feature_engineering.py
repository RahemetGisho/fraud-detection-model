...
def calculate_country_risk(df: pd.DataFrame, target_series: Optional[pd.Series] = None) -> pd.DataFrame:
    """
...
def engineer_behavior_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["time_since_prev_txn"] = df["time_since_prev_txn"].fillna(NO_PRIOR_TRANSACTION_SENTINEL_SECONDS)
