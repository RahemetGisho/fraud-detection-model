...
def transform_fraud_data(
    ...
    df.drop(
        columns=[c for c in _FRAUD_DROP_COLS if c in df.columns],
        inplace=True,
        errors="ignore",
    )
    ...
    if fit:
        ...
        if scaler is None:
            raise ValueError(
                "An instantiated training scaler must be provided for test transform."
            )
    ...
    if fit:
        ...
        if train_columns is None:
            raise ValueError(
                "train_columns must be provided for test transform alignment."
            )
...
def transform_creditcard(
    df: pd.DataFrame, scaler: Optional[StandardScaler] = None, fit: bool = True
